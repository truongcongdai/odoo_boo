...
class AdvancedIntegrateSaleOrderLazada(http.Controller):
                ...
                if data['message_type'] == 0:
                    if 'order_status' in data['data']:
                        try:
                            ...
                            if len(order_id) > 0:
                                # Status order
                                self.create_order_queue(order_id=order_id, data=data)
                            else:
                                try:
                                    # create Order Lazada
                                    self.create_order_queue(order_id=order_id, data=data)
    ...
    def create_order_queue(self, order_id, data):
        if data:
            s_mkp_order_type = 'create'
            if order_id:
                s_mkp_order_type = 'update'
            return request.env['s.mkp.order.queue'].sudo().create(
                {
                    's_mkp_payload': data,
                    's_mkp_order_id': data['data'].get('trade_order_id'),
                    's_is_mkp_lazada': True,
                    's_mkp_order_type': s_mkp_order_type
                }
            )
