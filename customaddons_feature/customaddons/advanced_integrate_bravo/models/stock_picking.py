...
class StockMove(models.Model):
    ...
    def button_validate(self):
        ...
        for rec in self:
            if rec.sale_id and rec.sale_id.is_magento_order and rec.sale_id.sale_order_status == 'hoan_thanh_1_phan':
                if rec.transfer_type == 'in':
                    if rec.state == 'done':
                        if rec.sale_id.s_date_completed:
                            s_comp_date = rec.sale_id.s_date_completed
                            rec.sale_id.write({
                                's_date_completed': False,
                                'completed_date': s_comp_date
                            })
        return res
                ...
                if (len(
                    ...
                    if line.sale_line_id.product_uom_qty != 0 and line.product_id.detailed_type == 'product':
                        ...
                        vals = {
                            ...
                            'quantity_done': quantity_done if quantity_done else 0,
                            'product_cost': round(
                                int(line.sale_line_id.s_lst_price)) if line.sale_line_id.s_lst_price else 0,
                            'so_line_price': 0,
                        }
                        if is_magento_order:
                            ...
                            vals.update({
                                ...
                                'so_line_price_subtotal': round(
                                    int(line.sale_line_id.s_lst_price)) * int(quantity_done) - so_line_discount,
                            })
                        else:
                            if 0 < line.sale_line_id.price_unit < line.sale_line_id.s_lst_price:
                                total_line_price = round(line.sale_line_id.s_lst_price) * int(quantity_done)
                            else:
