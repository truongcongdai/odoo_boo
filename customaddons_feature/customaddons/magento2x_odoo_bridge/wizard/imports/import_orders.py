...
class ImportOrders(models.TransientModel):
    ...
    def import_products(self,product_tmpl_ids, channel_id, sdk, kwargs):
        ...
        mapped = channel_id._match_mapping(mapping_obj,domain,limit=None).mapped('store_variant_id')
        product_tmpl_ids=list(set(product_tmpl_ids)-set(mapped))
        if len(product_tmpl_ids):
            feed_domain = [('store_id', 'in',product_tmpl_ids)]
            product_feeds = channel_id.match_product_feeds(domain=feed_domain,limit=None).mapped('store_id')
            
            product_tmpl_ids=list(set(product_tmpl_ids)-set(product_feeds))
            if len(product_tmpl_ids):
                feed_domain = [('store_id', 'in',product_tmpl_ids)]
                product_variant_feeds = channel_id.match_product_variant_feeds(domain=feed_domain,limit=None).mapped('store_id')
        ...
        if len(product_tmpl_ids):
            ...
            for i in product_tmpl_ids:
                product_data = channel_id._fetch_magento2x_product_data(sdk,filter_on="on_id",id=i,fields='items[id,sku],total_count')
                if product_data.get('total_count'):
                    product_data = isinstance(product_data.get('data',{}).get('items',{}), list) and product_data.get('data',{}).get('items',{})[0]
                    if not product_data:
                        message += "product id {} not received".format(str(i))
                        break
                    ...
                    data = data.get('data')#isinstance(data.get('data',{}).get('items',False),list) and data.get('data',{}).get('items',False)[0]
                    if not data:
                
                        break
                    data = import_product_obj._magento2x_import_product(sdk,channel_id,
                                                product_id,data,kwargs,
    ...
    def magento2x_get_order_line(self,order_id,carrier_id,order_data):
        ...
        message=''
        default_tax_type = self.channel_id.default_tax_type
        lines=[]
        lines += [(5,0,0)]
        if order_items:
            shipping_amount = order_data.get('shipping_incl_tax', 0.00)
    ...
    def get_order_vals(self,sdk,increment_id,status,order_data):
        ...
        server_tz = datetime.now(tzlocal()).tzname()
        ...
        if order_data.get('items'):
            ...
            vals = dict(
                ...
                date_order = order_data.get('created_at') + " "+ server_tz,
                confirmation_date = order_data.get('created_at') + " "+ server_tz,
            )
    ...
    def _magento2x_import_orders(self, sdk, store_id, channel_id, kwargs):
        ...
        if data and data.get('items'):
            ...
            for item in items:
                if item.get('entity_id') in ignore_ids:
                    failed_sync_prod.append(item.get('increment_id') + ",")
                    continue
                if item.get('store_id')!=store_id:
                    failed_sync_store.append(
                        item.get('increment_id') + " store id: " + str(item.get('store_id')) + ", ")
                    continue
        ...
        if kwargs.get('failed_sync_store'):
            kwargs['failed_sync_store'].extend(failed_sync_store)
        else:
            kwargs.update(failed_sync_store=failed_sync_store)
        if kwargs.get('failed_sync_prod'):
            kwargs['failed_sync_prod'].extend(failed_sync_prod)
        else:
            kwargs.update(failed_sync_prod=failed_sync_prod)
        if last_page:
                if kwargs.get('failed_sync_store'):
                    total_failed = len(kwargs.get('failed_sync_store')) + len(kwargs.get('failed_sync_prod'))
                    message += "Records {} Failed to import. <br>".format(total_failed)
                    message += 'Order(s) '+''.join(map(str,kwargs.get('failed_sync_store')))
                    message += " ignored due to different store ids." \
                            + " config store ID:" \
                            + str(store_id) \
                            + " Maybe this will get resolved if you configure another connection with different store." \
                            + "<br>"
                if kwargs.get('failed_sync_prod'):
                    message += 'Orders(s) '+''.join(map(str,kwargs.get('failed_sync_prod')))
                    message += " ignored due to different store ids." \
                            + " config store ID:" \
                            + str(store_id) \
                            + " Maybe this will get resolved if you configure another connection with different store." \
        ...
        if not last_page and len(import_res) < magento_page_size:
            kwargs.update(page_size=len(import_res))
        return import_res
