odoo.define('advanced_loyalty_program.TicketScreen', function (require) {
    'use strict';
    ...
    const TicketScreenInherit = (TicketScreen) => class extends TicketScreen {
            ...
            if (orderline.id in this.env.pos.toRefundLines) {
                ...
                const newToRefundDetail = {
                    ...
                    orderline: {
                        ...
                        loyalty_points: orderline.loyalty_points,
                        s_is_loyalty_reward_line: orderline.s_is_loyalty_reward_line,
                    },
        ...
        async _onDoRefund() {
            ...
            for (let i = 0; i < allToRefundDetails.length; i++) {
                ...
                ///Nếu line là sản phẩm được tặng
                    ...
                    ///TH2: CTKM tặng 1 sản phẩm
                        ...
                        for (let l = 0; l < this.env.pos.promo_programs.length; l++) {
                            if (allToRefundDetails[i].orderline.productId === this.env.pos.promo_programs[l].discount_line_product_id[0] && this.env.pos.promo_programs[l].reward_type === 'product') {
                                total_price_free_product += allToRefundDetails[i].orderline.price * allToRefundDetails[i].qty
                            }
                        }
                    }
                    ...
                    if (coupon_price > 0) {
                        ...
                        if (!autoRefund.length && order.orderlines.models[i].quantity > 0) {
                            allToRefundDetails.push({
                                "qty": order.orderlines.models[i].quantity, "orderline": {
            ...
            const destinationOrder = this.props.destinationOrder && customer === this.props.destinationOrder.get_client() ? this.props.destinationOrder : this.env.pos.add_new_order({silent: true});

            // Add orderline for each toRefundDetail to the destinationOrder.
            var reward_detail = [];
            var refund_all = true;
            for (const refundDetail of allToRefundDetails) {
                if (!refundDetail.orderline.s_is_loyalty_reward_line) {
                    const product = this.env.pos.db.get_product_by_id(refundDetail.orderline.productId);
                    if (refundDetail.orderline.qty !== refundDetail.qty) {
                        refund_all = false;
                    }
                    refundDetail.orderline.loyalty_points = refundDetail.orderline.loyalty_points * refundDetail.qty / refundDetail.orderline.qty;
                ...
                // Return đơn hàng sử dụng logic customize
            }
            if (refund_all){
                if (order.orderlines.models.length > 0) {
                    for (let i = 0; i < order.orderlines.models.length; i++) {
                        if (order.orderlines.models[i].s_is_loyalty_reward_line === true) {
                            const product = this.env.pos.db.get_product_by_id(order.orderlines.models[i].product.id);
                            const options = this._prepareRefundOrderlineOptions({
                                qty: order.orderlines.models[i].quantity,
                                orderline: {
                                    id: order.orderlines.models[i].id,
                                    productId: order.orderlines.models[i].product.id,
                                    ...
                                    tax_ids: [],
                                    discount: 0,
                                    loyalty_points: order.orderlines.models[i].loyalty_points,
                                    s_is_loyalty_reward_line: true,
                                }
                            });
                            await destinationOrder.add_product(product, options);
                            }
            ...
            // if (typeof (this.env.pos.config.module_pos_loyalty) !== "undefined" && this.env.pos.config.module_pos_loyalty && this.env.pos.config.loyalty_id) {
            //     if (this.env.pos.loyalty.is_refund_loyalty_points) {
            //         var reward = await this.rpc({
            //             model: 'pos.order.line',
            //             method: 'refund_loyalty_points',
            //             args: [0, allToRefundDetails, refund_all],
            //         })
            //         reward_detail = reward;
            //     }
            // }
            // if (reward_detail.length > 0) {
            //     for (var i = 0; i < destinationOrder.orderlines.models.length; i++) {
            //         if (refund_all) {
            //             var refundReward = []
            //             for (var j = 0; j < reward_detail.length; j++) {
            //                 if (destinationOrder.orderlines.models[i].refunded_orderline_id === reward_detail[j].refunded_orderline_id) {
            //                     destinationOrder.orderlines.models[i].loyalty_points = reward_detail[j].loyalty_points;
            //                 }
            //             }
            //         } else {
            //             for (var j = 0; j < reward_detail.length; j++) {
            //                 if (destinationOrder.orderlines.models[i].refunded_orderline_id === reward_detail[j].refunded_orderline_id) {
            //                     destinationOrder.orderlines.models[i].loyalty_points = reward_detail[j].loyalty_points;
            //                     // if (reward_detail[j].s_is_loyalty_reward_line === true) {
            //                     //     pass
            //                     //     // delete destinationOrder.orderlines.models[i]
            //                     // }
            //                 }
            //             }
            //         }
        ...
        _getSearchFields() {
            return Object.assign({}, super._getSearchFields(), {
                PHONE: {
                    displayName: this.env._t('Phone'), modelField: 'partner_id.phone',
                },
            });
        }
    };
