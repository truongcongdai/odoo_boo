...
class SLoyaltyPosOrderLineInherit(models.Model):
    ...
    def _compute_type_discount_pos(self):
        res = super(SLoyaltyPosOrderLineInherit, self)._compute_type_discount_pos()
        for rec in self:
            if rec.s_is_loyalty_reward_line and rec.s_loyalty_program_id and rec.s_loyalty_program_id.reward_type == 'point':
                ...
                if not line.product_id.available_in_pos and not line.is_reward_product() and line.product_id.detailed_type == 'service':
                    if line.refunded_orderline_id or (
                        ...
                        if line.refunded_orderline_id and line.qty > 0 and rec.order_id.is_cancel_order:
                            ...
                            if line.program_id and line.program_id.reward_type == 'discount' and line.program_id.s_type_discount and line.product_id.detailed_type == 'service':
                                if line.program_id.s_type_discount == 'chanel':
                                    refund_chanel_discount += line.price_subtotal_incl
                                elif line.program_id.s_type_discount == 'crm':
                                    refund_crm_discount += line.price_subtotal_incl
                                elif line.program_id.s_type_discount == 'hr':
                                    refund_hr_discount += line.price_subtotal_incl
                            elif line.s_is_loyalty_reward_line and line.s_loyalty_program_id and line.s_loyalty_program_id.reward_type == 'point':
                                refund_crm_discount += line.price_subtotal_incl
                        ...
                        if line.program_id and line.program_id.reward_type == 'discount' and line.program_id.s_type_discount and line.product_id.detailed_type == 'service':
                            if line.program_id.s_type_discount == 'chanel':
                                is_chanel_discount += line.price_subtotal_incl
                            elif line.program_id.s_type_discount == 'crm':
                                is_crm_discount += line.price_subtotal_incl
                            elif line.program_id.s_type_discount == 'hr':
                                is_hr_discount += line.price_subtotal_incl
                        elif line.s_is_loyalty_reward_line and line.s_loyalty_program_id and line.s_loyalty_program_id.reward_type == 'point':
                            is_crm_discount += line.price_subtotal_incl
                ...
                # ctkm, coupon, giftcard
                    ...
                    if not line.is_reward_product():
                        if line.refunded_orderline_id or (line.sale_order_line_id and line.qty < 0):
                            ...
                            total_discount += line.price_subtotal_incl
                            if line.is_line_gift_card and not line.gift_card_id.is_not_calculate_amount and line.gift_card_id.s_type_discount:
                                if line.gift_card_id.s_type_discount == 'chanel':
                                    is_chanel_discount += line.price_subtotal_incl
                                elif line.gift_card_id.s_type_discount == 'crm':
                                    is_crm_discount += line.price_subtotal_incl
                                elif line.gift_card_id.s_type_discount == 'hr':
                                    is_hr_discount += line.price_subtotal_incl
                            elif line.program_id and line.program_id.reward_type == 'discount' and line.program_id.s_type_discount and line.product_id.detailed_type == 'service':
                                if line.program_id.s_type_discount == 'chanel':
                                    is_chanel_discount += line.price_subtotal_incl
                                elif line.program_id.s_type_discount == 'crm':
                                    is_crm_discount += line.price_subtotal_incl
                ...
                if line.is_free_product:
                    ...
                    if not line.program_id and not line.is_line_gift_card and not line.is_refund_gift_card() and not line.s_is_loyalty_reward_line:
                        ...
                        line.update({
                            ...
                            's_chanel_discount': round(chanel_discount),
                            's_crm_discount': round(crm_discount),
                            's_hr_discount': round(hr_discount),
                        })
                        if line.qty < 0:
                            line.update({
                                'boo_phan_bo_price_total': line.price_subtotal_incl - boo_total_discount_percentage,
                                ...
                                's_chanel_discount': -chanel_discount if chanel_discount > 0 else chanel_discount,
                                's_crm_discount': -crm_discount if crm_discount > 0 else crm_discount,
                                's_hr_discount': -hr_discount if hr_discount > 0 else hr_discount,
                            })
                        ...
                        list_order_line.append(
                            {
                                ...
                                's_is_loyalty_reward_line': line.s_is_loyalty_reward_line,
                            }
                        )
