...
class MultiChannelSale(models.Model):
    ...
    def get_store_attribute_value_id(self, name, attribute_id, create_obj=False):
        current_attribute = self.env['product.attribute'].sudo().browse(attribute_id)
        if current_attribute.type == 'color':
            name = name.replace('#', '')
            domain = [
                ('code', '=', name),
        ...
        match = self.env['product.attribute.value'].search(domain, limit=1)
        if (not match) and create_obj:
