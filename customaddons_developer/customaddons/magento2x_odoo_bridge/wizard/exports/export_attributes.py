...
class exportmagento2xattributes(models.TransientModel):
    ...
    def magento2x_get_attribute_data(self, attribute_id, channel_id):
        ...
        current_attribute_mapping = self.env['channel.attribute.mappings'].sudo().search(
            [('odoo_attribute_id', '=', attribute_id.id)], limit=1)
        if current_attribute_mapping:
            data['attribute_code'] = current_attribute_mapping.store_attribute_name
        else:
            data['attribute_code'] = "odoo_" + str(attribute_id.id)
    ...
    def magento2x_update_attribute_data(self, sdk, channel_id, attribute_id):
        ...
        if not match:
            ...
            for option in options:
                ...
                # if data.get('data') and attribute_id.type == 'color':
                #     if option.get('label'):
                #         option_value = option.get('label')
                #         product_attribute_value_id = self.env['product.attribute.value'].sudo().search(
                #             [('code', '=', option_value)])
                #         if product_attribute_value_id:
                #             attribute_value_mappings = self.env['channel.attribute.value.mappings'].create(
                #                 {
                #                     'store_attribute_value_id': data.get('data'),
                #                     'store_attribute_value_name': option.get('label'),
                #                     'attribute_value_name': product_attribute_value_id.id,
