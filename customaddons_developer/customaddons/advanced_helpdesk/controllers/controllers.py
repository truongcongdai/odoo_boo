                ...
                if body['object'] == 'page':
                    ...
                    if entries:
                        for entry in entries:
                            webhookEvent = entry.get('messaging')
                            if webhookEvent:
                                search_fb_message_id = False
                                fb_message_id = False
                                if webhookEvent[0].get('message'):
                                    fb_message = webhookEvent[0].get('message')
                                ...
                                sender = webhookEvent[0].get('sender')
                                if sender:
                                    ...
                                    if senderPsid:
                                        ...
                                        if not channel or not partner or not ticket:
                                            if webhookEvent[0].get('message'):
                                                message_text = webhookEvent[0].get('message').get('text')
                                            else:
                                                message_text = False
                                            ...
                                            if not search_fb_message_id:
                                                channel.message_post(
                                                    ...
                                                    body=webhookEvent[0].get('message').get('text'),
