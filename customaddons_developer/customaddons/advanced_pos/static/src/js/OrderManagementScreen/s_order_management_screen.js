odoo.define('advanced_pos.SaleOrderManagementScreen', function (require) {
    'use strict';
    ...
    const SSaleOrderManagementScreen = (SaleOrderManagementScreen) => class extends SaleOrderManagementScreen {
        constructor() {
            super(...arguments);
        }
              ...
              if (selectedOption){
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    if (line.qty_delivered !== 0){
                        real_price = Math.round(line.price_total/line.product_uom_qty)
                    }
                    if(this.env.pos.db.get_product_by_id(line.product_id[0]).type === 'product'){
                        var real_price = line.price_unit - line.boo_total_discount_percentage/line.product_uom_qty
                    }
                    if (line.is_loyalty_reward_line){
                        real_price = 0
                    }

                    // check line shipping: price = 0
                    try {
                        let is_shipping_line = await this.rpc({
                            model: "sale.order.line",
                            method: 'check_is_shipping_line',
                            args: [line.id],
                        });
                        if (is_shipping_line) {
                            real_price = 0
                    ...
                    // The Anh Custom
                    // let sale_order_id = await this.rpc({
                    //     model: 'sale.order',
                    //     method: 'search_read',
                    //     domain: [['id', '=', new_line.sale_order_origin_id.id], ['payment_method', '=', 'online']],
                    // });
                    // if (sale_order_id.length > 0){
                    //     line['order_id_payment_online'] = true
                    // }
                    let is_line_coupon_program = await this.rpc({
                        model: "sale.order.line",
                        method: 'check_is_line_coupon_program',
                        args: [line.id],
                    });
                    if (is_line_coupon_program) {
                        continue;
                    }
                    let is_line_product_free = await this.rpc({
                        model: "sale.order.line",
                        method: 'check_is_line_product_free',
                        args: [line.id],
                    });
                    if (is_line_product_free) {
                        real_price = 0
                    }
                    // The Anh Custom
                    new_line.setQuantityFromSOL(line);
                    new_line.set_unit_price(real_price);
                ...
                if (this.env.pos.config.down_payment_product_id){
                    ...
                    const body = this.env._t(
                        "Có vẻ như bạn đã không định cấu hình sản phẩm trả trước tại điểm bán hàng của mình. " +
                        "Bạn có thể vào cấu hình điểm bán hàng của mình để chọn một."
                    );
    ...
    Registries.Component.extend(SaleOrderManagementScreen, SSaleOrderManagementScreen);
    return SSaleOrderManagementScreen;
});