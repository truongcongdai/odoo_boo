...
const MainMenu = AbstractAction.extend({
    ...
    events: {
        "click .button_logistics": function () {
            this.do_action('advanced_logistic.s_logistic_tracking_action');
        },
        ...
        "click .o_stock_mobile_barcode": async function () {
            ...
            if (barcode) {
                this._rpc({
                    'model': 'stock.picking',
                    'method': 'search',
                    'args': [[['logistic_barcode', '=', barcode]]],
                }).then(result => {
                    const s_transfer_out_id = result
                    this._rpc({
                        model: 'stock.picking',
                        method: "create_s_logistic_tracking",
                        args: [result],
                    }).then(value => {
                        // return this.do_action('advanced_logistic.s_logistic_tracking_action');
                        if (value) {
                            return this.do_action({
                                name: _t('Logistic Tracking'),
                                type: 'ir.actions.act_window',
                                res_model: 's.logistic.tracking',
                                domain: value ? [['id', '=', value]] : [],
                                views: [[false, 'kanban']],
                            });
                        }
                    });
