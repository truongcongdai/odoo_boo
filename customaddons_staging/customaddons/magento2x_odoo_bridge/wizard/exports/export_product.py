...
class ExportMagento2xProducts(models.TransientModel):
    ...
    def magento2x_get_store_category_ids(self, template_id, channel_id):
        ...
        if template_id.channel_category_ids:
            channel_categ_ids = template_id.channel_category_ids.filtered(
                lambda categ: categ.instance_id.id == channel_id.id).mapped('extra_category_ids')
        ...
        for attribute_value_id in template_id.product_variant_ids.mapped('product_template_attribute_value_ids'):
            ...
            if attribute_match:
                if attribute_id not in set_attribute_ids:
                    result['message'] += '<br/> Attribute(%s) not related with attribute set (%s)' % (
                        attribute_id.name, product_set_id.set_name)
                    return result
                ...
                if value_match:
                    ...
                    result['message'] += '<br/> No  attribute value mappping found for %s ==> %s.' % (
                        attribute_id.name, attribute_value_id.name)
                    return result
            else:
                result['message'] += '<br/>No attribute mappping found for %s ==> %s.' % (
                    attribute_id.name, attribute_value_id.name)
                return result
        result['data'] = data
        return result
    ...
    def get_magento2x_custom_attributes(self, product_id, channel_id, **kwargs):
        ...
        for value_id in product_id.product_template_attribute_value_ids:
            ...
            if attribute_match:
                ...
                value_match = channel_id.match_attribute_value_mappings(
                    attribute_value_id=value_id.product_attribute_value_id.id)
                if value_match:
                    temp_data['value'] = int(value_match.store_attribute_value_id)
                    data += [temp_data]
                else:
                    result['message'] += '<br/> No  attribute value mappping found for %s  ==> %s.' % (
                        attribute_id.name, value_id.name)
                    return result
            else:
                result['message'] += '<br/> No  attribute  mappping found for %s ==> %s.' % (
                    attribute_id.name, value_id.name)
    ...
    def magento2x_get_product_data(self, _type, product_id, channel_id,
        ...
        product_data = product_id.with_context(context)[0].read([])[0]
        ...
        if not product_id.default_code:
            product_id.default_code = channel_id.sku_sequence_id.next_by_id() if channel_id.sku_sequence_id \
                else "oid_" + str(product_id.id)
            if _type == 'configurable':
                product_id.default_code = product_id.ma_san_pham
                product_id.custom_m2_default_code = product_id.ma_san_pham if product_id.ma_san_pham \
                    else "oid_" + str(product_id.id)
            # else product_id.name + "_oid_" + str(product_id.id)
        sku = product_id.default_code
        # start logan
        if _type == 'configurable':
            # Doi voi san pham cha, khong co SKU lay MSP lam product code
            # xu ly truong hop data cu theo sku_sequence_id: Ecom-
            if channel_id.sku_sequence_id.prefix in product_id.default_code:
                # if product_id.custom_m2_default_code:
                product_id.default_code = product_id.ma_san_pham
                product_id.custom_m2_default_code = product_id.ma_san_pham if product_id.ma_san_pham \
                    else "oid_" + str(product_id.id)
                # product_id.custom_m2_default_code = channel_id.sku_sequence_id.next_by_id() if channel_id.sku_sequence_id \
                #     else "oid_" + str(product_id.id)
                sku = product_id.custom_m2_default_code
            if len(product_id.product_variant_ids) == 1:
                sku = product_id.ma_san_pham
                # product_id.default_code = product_id.ma_san_pham
                product_id.custom_m2_default_code = product_id.ma_san_pham
        # end logan
        ...
        data = dict(
            ...
            price=product_data.get('price') if product_data.get('price') > 0 else product_id.list_price,
            weight=product_id.weight,
            status=product_id.sale_ok and 1 or 0,
            visibility=4,
        ...
        if _type == 'configurable':
            if kwargs.get('match'):
                ...
                if product_id:
                    # data['sku'] = product_id.default_code
                    if product_id.default_code and len(product_id.product_variant_ids) > 1:
                        data['sku'] = product_id.default_code
                    else:
                        data['sku'] = product_id.custom_m2_default_code
                else:
                    data['sku'] = channel_id.sku_sequence_id.next_by_id() if channel_id.sku_sequence_id \
                        else "oe_template_" + str(product_id.id)
                    product_id.default_code = data['sku']
                # data['sku'] = channel_id.sku_sequence_id.next_by_id() if channel_id.sku_sequence_id \
                #     else "oe_template_" + str(product_id.id)
                # product_id.default_code = data['sku']
                # Mac dinh app
                # else product_id.name + "oe_template_" + str(product_id.id)
        ...
        if data:
            ...
            if store_template_id:
                ...
                if self._context.get('base_operation') == "update":
                    result[
                        'update_simple'] = "done"  # returning true as no store id required Update required scope can also be added
                if operation != 'update':
        ...
        if template_id.attribute_line_ids:
            ...
            for product_id in template_id.product_variant_ids:
                ...
                if (operation != 'export') and (
                        not channel_id.match_product_mappings(domain=[('product_name', '=', product_id.id)])):
                    result['message'] += _(
                        '<br/>New product variant can not added while template update (product %r(%r) )' % (
                ...
                if data:
                    ...
                    if res.get('message', ''):
                        ...
                        result['message'] += '<br/> Variant %s(%s) Operation(%s)' % (
                            product_id.name, product_id.id, operation)
                    data = res.get('data')
            ...
            if data:
                ...
                # start logan
                sku = None
                current_configurable_product_mapping = self.env['channel.template.mappings'].sudo().search(
                    [('odoo_template_id', '=', template_id.id)], limit=1)
                if current_configurable_product_mapping:
                    sku = data['sku']
                # end logan
    ...
    def post_magento2x_category(self, sdk, channel_id, odoo_category_ids, operation='export', **kwargs):
        ...
        for category in odoo_category_ids:
            mag_cat_id = self.env['export.categories'].magento2x_post_categories_data(sdk, channel_id, category,
                                                                                      operation="export")
            if mag_cat_id:
    ...
    def export_mage2x_product_attribute(self, sdk, product_tmpl_ids, channel_id):
        ...
        attribute_value_ids = product_tmpl_ids.mapped('product_variant_ids').mapped(
    ...
    def magento2x_export_templates(self):
        ...
        if len(exclude_type_ids):
            message += '<br/> Total %s  product template not exported/updated because of having type other than product  and consumable .' % (
                len(exclude_type_ids))
