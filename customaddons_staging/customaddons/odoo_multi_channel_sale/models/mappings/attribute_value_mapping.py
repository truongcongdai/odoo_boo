...
class ChannelAttributeValueMappings(models.Model):
    ...
    attribute_value_name = fields.Many2one(comodel_name='product.attribute.value', string='Odoo Attribute Value Name',
                                           required=True
                                           )
    odoo_attribute_value_id = fields.Integer('Odoo Value ID', required=True)


    def _compute_attribute_value_code(self):
        for rec in self:
            rec.attribute_value_code=None
            if rec.attribute_value_name:
                rec.attribute_value_code = rec.attribute_value_name.code
