...
class CustomerSegmentAnalysis(models.TransientModel):
    ...
    rfm_score_id = fields.Many2one(comodel_name="setu.rfm.score", string="RFM Scores")
    rfm_segment_id = fields.Many2one(comodel_name="setu.rfm.segment", string="RFM Segment")

    days_from_last_purchase = fields.Integer(string="Days from last purchase")
    total_orders = fields.Integer(string="Total Orders")
    total_revenue = fields.Float(string="Total Revenue")
    total_order_percentage = fields.Float(string="Total Orders (%)")
    total_revenue_percentage = fields.Float(string="Total Revenue (%)")
